...
DATA_DIR="${1:-${ROOT_DIR}/stocks_synthetic_dataset}"
OUT_FILE="${2:-${DATA_DIR}/balanced_dataset_merged_depth.jsonl}"
...
with out_file.open("w", encoding="utf-8") as fout:
    for depth in depths:
        src = data_dir / f"balanced_dataset_single_{depth}.jsonl"
        if not src.exists():
