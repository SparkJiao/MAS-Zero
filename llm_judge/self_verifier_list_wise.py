...
def majority_vote_funct(responses, extracted_answers, max_response_per_sample, dataset):
    answer_list = []
    extracted_list = []
...
def multiple_choice_funct(responses, extracted_answers, max_response_per_sample, dataset, post_process_path, sampler):
    ...
    if 'gpt' in post_process_path:
        ...
        output_fields_and_description = {key: f"Your {key}." if not 'selection' in key else f"Your {key}. {output_description}" for key in
                                         ['thinking', 'selection']}
        ...
        output_fields_and_description = '\n'.join(
            [f"<{key}> [Your {key}.] </{key}>" if not 'selection' in key else f"<{key}> [Your {key}. {output_description}] </{key}>\n" for key in
             ['thinking', 'selection']])
        system_prompt = 'You are a judge to select the best answer from a list of candidate answers. ' + FORMAT_INST(output_fields_and_description)
...
def run_self_verifier(post_process_path, log_path, score_path, responses, sampler, post_processer, extracted_answers, dataset, max_response_per_sample,
                      majority_vote=False, multiple_choice=False):
    if majority_vote:
        return majority_vote_funct(responses, extracted_answers, max_response_per_sample, dataset)
    if multiple_choice:
        return multiple_choice_funct(responses, extracted_answers, max_response_per_sample, dataset, post_process_path, sampler)

    print(f"Run self verifier. log_path: {log_path}, post_process_path: {post_process_path}, score_path: {score_path}", flush=True)
